...
# The method I use is dilating and blurring the image and getting the difference between
...
# I chose to convert it to grayscale instead of using the L channel of the LAB color space 
# because the results were the same but using grayscale is easier.
gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

# Create a clahe object so that we can apply histogram equalization.
...
# Now we apply a smoothing bilateral filter in order to get rid of some of the noisy black spots that isn't text.
...
# I got the method for dilating, blurring and canceling out the background in the SO answer below.
...
# At this point we're left with a blurry image that has the background of the image without the text.
...
# Now we normalize the image to increase the contrast and make the text stand out more
